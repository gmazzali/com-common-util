...
import com.common.util.app.context.ApplicationContextHolder;
...
public class ConfigurableBean implements Serializable {
	...
	public static <T> void initializeBean(T bean) {
		...
		for (Field autowiredField : autowiredFields.values()) {
			...
			try {
				ApplicationLogger.debug("Inyectando en el bean " + bean.getClass().getName() + " la propiedad " + autowiredField.getName()
						+ " por tipo");
				...
				ApplicationLogger.debug("Error al intentar autowire por tipo " + autowiredField.getName(), e);
				try {
					ApplicationLogger.debug("Inyectando en el bean " + bean.getClass().getName() + " la propiedad " + autowiredField.getName()
							+ " por nombre");
					...
					ApplicationLogger.debug("Error al intentar autowire por nombre " + autowiredField.getName(), e);
				}
			}
		}
	}
